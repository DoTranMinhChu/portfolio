    ...
    <>
      ...
      <section className="bg-gray-800">
        <div className="p-4 py-12 sm:p-8 md:p-24 xl:px-48 sm:py-18 md:py-24">
          ...
          <h2 className="pt-2 pb-8 text-4xl font-bold text-gray-300">
            {project.appType}
          </h2>
          <h3
            className="py-2 text-lg text-gray-400"
            dangerouslySetInnerHTML={{ __html: project.shortDescription }}
          ></h3>
        </div>
      </section>
      <section className="flex items-center justify-center">
        <img className="w-full" src={project.thumbnailUrl}></img>
      </section>
        ...
        <div className="grid grid-cols-1 gap-12 p-4 py-12 sm:p-8 md:p-24 xl:px-48 sm:py-18 md:py-24 md:grid-cols-1 lg:grid-cols-9">
                ...
                <div className="p-1 my-1 text-xl font-semibold text-gray-300">
                  {Object.keys(project.teamSize).map((value: string) => {
                    if (project.teamSize[value])
                      return (
                        <h2 key={`${project.teamSize[value]}-${value}`}>
                          {project.teamSize[value]} - {value}
                        </h2>
                      );
                  })}
                </div>
          ...
          <div className="col-span-full lg:col-span-6">
            <div>
              ...
              <div
                className="p-1 my-1 text-base text-gray-300"
                dangerouslySetInnerHTML={{ __html: project.fullDesciption }}
              ></div>
            </div>
          </div>
        </div>
...
): Promise<GetStaticPropsResult<IStaticPropsProject>> => {
